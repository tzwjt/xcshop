...
class Notify
{
    ...
    */
     ...
     */
    private function getShopNotifyInfo($shop_id=0){
    
        $website_model=new SiteInfoModel();
        $website_obj=$website_model->getInfo("1=1", "web_shop_site_name");
        if(empty($website_obj)){
            $this->shop_name="美肤日志SKINDAY商城";
        }else{
            $this->shop_name=$website_obj["web_shop_site_name"];
        }
     ...
     * 查询模板的信息
     */
    private function getTemplateDetail($shop_id=0, $template_code, $type, $notify_type = "user"){
    ...
    public function registAfter($params=null){
        ...
        $user_model=new MemberUserModel();
        ...
        if(empty($user_obj)){
          ...
          //  $user_name=$user_obj["user_name"];
            $user_name = $user_obj["login_phone"];
           // $nick_name=$user_obj["user_name"];
            $mobile=$user_obj["login_phone"];
        ...
        if(!empty($email) && $this->email_is_open==1){
            ...
            if(!empty($template_obj) && $template_obj["is_enable"]==1){
                ...
                $content=str_replace("{商城名称}", $this->shop_name, $content);
                $content=str_replace("{用户名称}", $user_name, $content);
                $send_title=$template_obj["template_title"];
                $send_title=str_replace("{商城名称}", $this->shop_name, $send_title);
    ...
    public function registBefor($params=null){
        ...
        if(!empty($mobile) && $this->mobile_is_open==1){
            ...
            $this->result["code"]=-1;
            $this->result["message"]="商城没有开启短信验证";
        }
        return $this->result;
    }
    ...
    public function registEmailValidation($params=null){
        ...
        if(!empty($email) && $this->email_is_open==1){
            ...
            $this->result["code"]=-1;
            $this->result["message"]="商城没有开启邮箱验证";
        }
        return $this->result;
    ...
    public function orderDelivery($params=null){
        Log::write("hook orderDelivery 000");
        #查询系统配置信息
        $this->getShopNotifyInfo(0);
        $order_goods_ids=$params["order_goods_ids"];
        Log::write("hook orderDelivery 001".$order_goods_ids);
        ...
        if(count($order_goods_str)>0){
            Log::write("hook orderDelivery 002".$order_goods_str[0]);
          ...
          //  Log::write("hook orderDelivery 003:".$order_id);
            $order_obj=$order_model->get($order_id);
            $buyer_id=$order_obj["buyer_id"];
            Log::write("hook orderDelivery 004:".$buyer_id);
            $user_obj = $user_model->get($buyer_id);
            $user_name=$user_obj["login_phone"];
            Log::write("hook orderDelivery 005:".$user_name);
          ...
          //  $order_no=$order_obj["out_trade_no"];
           ...
           // $mobile=$order_obj["receiver_mobile"];
            $mobile=$user_obj["login_phone"];
            Log::write("hook orderDelivery 006:".$mobile);
            $goods_express_model=new OrderGoodsExpressModel();
            $express_obj=$goods_express_model->getInfo(["order_id"=>$order_id, "order_goods_id_array"=>$order_goods_ids], "*");
            $express_company="";
            $express_no="";
            if (!empty($express_obj)) {
                Log::write("hook orderDelivery 0016 found");
                $express_company = $express_obj["express_company"];
                $express_no = $express_obj["express_no"];
            }
            Log::write("hook orderDelivery 007:".$express_company);
            Log::write("hook orderDelivery 008:".$express_no);
            $sms_params=array(
                "shop_name"=>$this->shop_name,
                "user_name"=>$user_name,
            ...
            Log::write("hook orderDelivery expresscompany:".$express_company);
            Log::write("hook orderDelivery expressno:".$express_no);
            #短信发送
            if(!empty($mobile) && $this->mobile_is_open==1){
              //  Log::write("hook orderDelivery 456:".$mobile);
                $template_obj=$this->getTemplateDetail($shop_id, "order_deliver", "sms");
                if(!empty($template_obj) && $template_obj["is_enable"]==1){
                  //  Log::write("hook orderDelivery 789:".$mobile);
                    $this->createNoticeSmsRecords($template_obj, $shop_id, $buyer_id, $mobile, $sms_params, "订单发货短信发送", 5);
                }
            }
            Log::write("hook orderDelivery 45www");
            // 邮件发送
            if (!empty($user_obj)) {
                $email = $user_obj["email"];
                if (! empty($email) && $this->email_is_open == 1) {
                    ...
                    if (! empty($template_obj) && $template_obj["is_enable"] == 1) {
                        ...
                        $content = str_replace("{物流公司}", $express_company, $content);
                        $content = str_replace("{快递编号}", $express_no, $content);
        ...
        if($send_type=="email"){
            ...
            if($this->email_is_open==1){
                if(!empty($template_obj) && $template_obj["is_enable"]==1){
                    ...
                    $this->result["code"]=-1;
                    $this->result["message"]="商城没有设置找回密码的模板!";
                }
            }else{
                $this->result["code"]=-1;
                $this->result["message"]="商城没开启邮箱验证!";
            ...
            if($this->mobile_is_open==1){
                if(!empty($template_obj) && $template_obj["is_enable"]==1){
                    ...
                    $this->result["code"]=-1;
                    $this->result["message"]="商城没有设置找回密码的短信模板!";
                }
            }else{
                $this->result["code"]=-1;
                $this->result["message"]="商城没开启短信验证!";
            }
        }
        return $this->result;
    }
    ...
    public function orderRemindBusiness($params=null){
        ...
        if(!empty($out_trade_no)){
            Log::write("hook out_trade_no: shop_id=".$shop_id.", out_trade_no=".$out_trade_no);
            //获取订单详情
            $order = new OrderModel();
            $order_detial = $order->getInfo(["out_trade_no"=>$out_trade_no]);
            //邮箱提醒
            ...
            if(!empty($mobile_array) && $template_sms_obj["is_enable"] == 1){
                ...
                foreach ($mobile_array as $v){
                    Log::write("hook out_trade_no: shop_id=".$shop_id.", out_trade_no=".$out_trade_no."v=".$v);
    ...
    public function orderRefoundBusiness($params=null){
        ...
        if(!empty($order_id)){
            //获取订单详情
            $order = new OrderModel();
            $order_detial = $order->getInfo(["id"=>$order_id]);
            //邮箱提醒
...
*/
    ...
    private function createNoticeSmsRecords($template_obj, $shop_id, $buyer_id, $mobile, $sms_params, $title, $records_type){
        $notice_handle=new NoticeHandle();
        $send_config=array(
            "appKey"=>$this->appKey,
        ...
        $notice_handle->createNoticeRecords($shop_id, $buyer_id, 1, $mobile, $title, json_encode($sms_params), $records_type, json_encode($send_config));
    }
    ...
    private function createNoticeEmailRecords($shop_id, $buyer_id, $mobile, $send_title, $content, $records_type){
        ...
        $notice_handle->createNoticeRecords($shop_id, $buyer_id, 2, $mobile, $send_title, $content, $records_type, json_encode($send_config));
    }
}
