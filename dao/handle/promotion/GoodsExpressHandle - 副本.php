...
class GoodsExpressHandle extends BaseHandle
{
    ...
    /**
     * 获取商品邮费总和
     * $is_able = 1表示固定运费,$is_able=0,计算运费
     */
    public function getSkuListExpressFee($goods_sku_list, $express_company_id, $province, $city, $district, $is_able)
    {
      //  $config = new Config();
        //查询用户是否选择物流
    ...
    private function getShippingFeeTemplate($express_company_id, $province, $city, $district = 0)
    {
        ...
        if (empty($temp)) {
            if (! empty($default)) {
                ...
                return  null; //NULL_EXPRESS_FEE; // 返回表示该地址不支持配送
            }
        } else {
            return $temp;
        }
     ...
     * @param unknown $city_id            
     */
    public function getExpressCompany( $goods_sku_list, $province_id, $city_id, $district_id)
    {
